        ...
        <div class="bg-white rounded-3xl shadow-xl shadow-gray-100 overflow-hidden border border-gray-100">
            ...
            <form action="{{ route('admin.users.store') }}" method="POST" class="p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <!-- Name -->
                    <div>
                        <label for="name" class="block text-sm font-medium text-gray-700 mb-2">ชื่อผู้ใช้งาน/ชื่อแผนก
                    ...
                    <!-- Department -->
                    <div>
                        <label for="dept_name" class="block text-sm font-medium text-gray-700 mb-2">แผนกที่สังกัด <span
                                class="text-red-500">*</span></label>
                        <select name="dept_name" id="dept_name" class="w-full text-sm" required>
                            <option value="">-- เลือกแผนก --</option>
                            @foreach ($departments as $dept)
                                <option value="{{ $dept }}" {{ old('dept_name') == $dept ? 'selected' : '' }}>
                                    {{ $dept }}</option>
                            @endforeach
                        </select>
                ...
                <div class="flex justify-end gap-3 pt-6 border-t border-gray-50">
                    <a href="{{ route('admin.users.index') }}"
                        class="px-6 py-3 border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50 font-medium text-sm transition">ยกเลิก</a>
                    <button type="submit"
                        class="px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 font-medium text-sm transition">สร้างบัญชีผู้ใช้</button>
                </div>
            </form>
        </div>
    </div>
